--[[
	...
	Date:		27.01.2019
	Author:	Mopar
	This is a port of SpamThrottle to work with TBC WoW, release 2.4.3 
...
function SpamThrottleMessage(visible, ...)
	...
	if (visible) then
		debugWin:AddMessage("SpamThrottle: " .. table.concat (tmp, " "), 0.5, 0.5, 1);
	end
	for i = 1, select('#', ...) do
		tmp[i] = nil
	end
...
function SpamThrottleMessageHex(visible, msg)
	...
	if Prat_UrlCopy then out = Prat_UrlCopy:RawLink(out) end
	if (visible) then
...
function UFOverHookEvents()
	if(time() - UFStartTime > 10 and UFInitialized == nil) then
		...
		if WIM_MessageEventHandler then 
			SpamThrottle_Orig_WIM_ChatFrame_OnEvent = WIM_MessageEventHandler;
			WIM_MessageEventHandler = SpamThrottle_WIM_ChatFrame_OnEvent;
			WIM_Present = true;
	...
	if arg2 then -- if this is not a server message
		...
		or event == "CHAT_MSG_TEXT_EMOTE") then
			...
			if event == "CHAT_MSG_CHANNEL" then
				for index, value in ipairs(this.channelList) do
